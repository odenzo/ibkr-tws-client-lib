...
import com.odenzo.base.OError

trait CatsEffectUtils {
  ...
  def exactlyOne[A](msg: String)(l: List[A]): IO[A] = {
    val err = OError.of(s"Requires List Size 1 but  ${l.length}: $msg")
  ...
  def optionOne[A](msg: String)(l: List[A]): IO[Option[A]] = {
    val err = OError.of(s"Expected List Size 0 or 1 but  ${l.length}: $msg")
  ...
  def required[A](msg: String)(o: Option[A]): IO[A] = {
    IO.fromOption(o)(OError.of(s"Required Value Missing: $msg"))
  }
  ...
  def requireNone[A](msg: String)(o: Option[A]): IO[Unit] = {
    o match {
      case None    => IO.unit
      case Some(v) => IO.raiseError[Unit](OError.of(s"Option Value Must Be Empty: $v -- $msg"))
    }
  }

  def nonEmptyListIO[A](msg: String)(l: List[A]): IO[NonEmptyList[A]] = {
    IO.fromOption(NonEmptyList.fromList(l))(OError.of(s"NEL Requires List of 1+ $msg"))
  }

}

object IOU extends CatsEffectUtils
