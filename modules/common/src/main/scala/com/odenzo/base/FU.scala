...
import _root_.com.odenzo.base.OError
import com.odenzo.base.OError.of
...
trait FU {
  ...
  def exactlyOne[F[_]: ApplicativeThrow, A](msg: String)(l: List[A]): F[A] = {
    val err = of(s"Requires List Size 1 but  ${l.length}: $msg")
  ...
  def optionOne[F[_]: ApplicativeThrow, A](msg: String)(l: List[A]): F[Option[A]] = {
    val err = of(s"Expected List Size 0 or 1 but  ${l.length}: $msg")
    if (l.length > 1) ApplicativeThrow[F].raiseError(err)
    else ApplicativeThrow[F].pure(l.headOption)
  }

  /** Raises an Option.empty to Error */
  def required[F[_]: ApplicativeThrow, A](msg: String)(o: Option[A]): F[A] = optionToError(o)(of(s"Required Value Missing: $msg"))
  ...
  def requireNone[F[_]: ApplicativeThrow, A](msg: String)(o: Option[A]): F[Unit] = {
    o match {
      ...
      case Some(v) => ApplicativeThrow[F].raiseError(of(s"Option Value Must Be Empty: $v -- $msg"))
    }
  }

  def nonEmptyListIO[F[_]: ApplicativeThrow, A](msg: String)(l: List[A]): F[NonEmptyList[A]] = {
    optionToError(NonEmptyList.fromList(l))(of(s"NEL Requires List of 1+ $msg"))
  }
